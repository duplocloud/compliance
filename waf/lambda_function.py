...
def create(event, context):
    ...
    create_tenant_index(host, awsauth)
    
    create_auth_index(host, awsauth)
...
def create_tenant_index(host, awsauth):

    url = 'https://' + host + '/tenant';
    headers = { "Content-Type": "application/json" }

    with open("custom/duplo_index.json") as f:
        template = f.read()

    requests.put(url, auth=awsauth, headers=headers, data=template)
    
def create_auth_index(host, awsauth):
...
def generate_wafacls_mapping():
    ...
    for webacl in webacls:
        logWebACLId = "arn:aws:wafv2:" + os.environ['REGION'] + ":" + os.environ['ACCOUNT_ID'] + ":global/webacl/" + webacl["Name"] + "/" + webacl["Id"];
